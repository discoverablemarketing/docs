...
<Note>
  **Using the API Explorer:** Click "Try It" on the right to test this endpoint interactively. Enter your API key from [app.getchatads.com/api/keys](https://app.getchatads.com/api/keys) in the Authorization header field.
</Note>
...
<ParamField body="message_analysis" type="string" default="balanced">
  ...
  - `fast` - Lightweight model extraction (~50ms)
  - `balanced` - Hybrid approach (~150ms)
  - `thorough` - Deeper extraction (~300ms)
</ParamField>
...
<ParamField body="skip_message_analysis" type="boolean" default="false">
  When `true`, skips keyword extraction and uses the message content directly as the search query.
