...
<Accordion title="NO_AFFILIATE_CREDENTIALS (422)" icon="link-slash">
**Cause:** No affiliate credentials configured for your team.

**Response details:**
```json
{
  "error": {
    "code": "NO_AFFILIATE_CREDENTIALS",
    "message": "No affiliate credentials configured"
  }
}
```

**Solution:**
Configure at least one of these in Settings → Integrations:
1. **Amazon PA-API Key** - For direct Amazon Product Advertising API access
2. **Amazon Affiliate Tag** - For appending your affiliate tag to Amazon URLs

Without affiliate credentials, ChatAds cannot generate monetizable links.
</Accordion>
...
<Accordion title="QUOTA_EXCEEDED (429)" icon="calendar">
...
- Check [status.getchatads.com](https://status.getchatads.com) if persistent
</Accordion>
...
When `success: true` but `data.filled: false`, the `reason` field explains why no affiliate link was inserted. This is a "soft failure"—the API processed the request successfully, but couldn't generate an affiliate link.

| Reason | Meaning | Action |
|--------|---------|--------|
| `below_intent_threshold` | Keyword found but intent score too low for your `min_intent` setting | Lower `min_intent` parameter or use clearer purchase language |
| `no_affiliate_link_found` | Product detected but no affiliate link available | Product may not be on supported retailers |
...
<Note>
The `reason` field is only present when `filled: false`. When `filled: true`, the affiliate link was successfully generated.
</Note>
...
4. **Message too short** - After sanitization (removing PII, URLs, blocked keywords), fewer than 2 words remain
   ```json
   // Example: "Buy this: user@email.com" becomes just "Buy this:" after sanitization
   // The API requires at least 2 words to extract product keywords
   ```
