...
| `FORBIDDEN` | 403 | Invalid API key | Check key in Settings → API Keys |
...
| `CONTENT_BLOCKED` | 422 | Blocked keyword detected | Review blocked keywords in Settings |
| `GEO_BLOCKED` | 422 | Country not allowed | Update geo restrictions |
| `NO_AFFILIATE_CREDENTIALS` | 422 | No affiliate credentials configured | Add Amazon API key or affiliate tag in Settings → Integrations |
| `DAILY_QUOTA_EXCEEDED` | 429 | Daily limit reached | Wait for reset at midnight UTC |
| `QUOTA_EXCEEDED` | 429 | Monthly limit reached | Upgrade plan |
| `INTERNAL_ERROR` | 500 | Server error | Retry or contact support |
| `RATE_LIMIT_UNAVAILABLE` | 503 | Rate limiter temporarily down | Retry in a few seconds |

<Tip>
Error responses don't count against your quota (except `DAILY_QUOTA_EXCEEDED` and `QUOTA_EXCEEDED`).
</Tip>
...
1. **Niche products** - Not available through our affiliate partners
