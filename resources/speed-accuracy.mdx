...
`fast` uses a lightweight model. `balanced` and `thorough` use deeper extraction with fallback.
</Accordion>

<Accordion title="What does skip_message_analysis do?">
When `skip_message_analysis: true`, ChatAds bypasses keyword extraction and uses your message content directly as the search query. This is useful when:
...
- `coverage` (default) - Full resolution chain with fallback sources
- `speed` - Skips fallback sources for faster response
...
1. **Use `message_analysis: "fast"`** - Lightweight extraction (~50ms vs ~150ms)
2. **Use `fill_priority: "speed"`** - Skip fallback sources
...
Affiliate links are resolved in real-time from our affiliate partners. Product availability and pricing reflect current data at the time of the request.
