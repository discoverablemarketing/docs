...
- `message_analysis` (optional): Keyword extraction method. Options: `"fast"` (lightweight ~50ms), `"balanced"` (default ~150ms), `"thorough"` (~300ms)
...
- `skip_message_analysis` (optional): Skip keyword extraction and use message directly as search query. Default: `false`
